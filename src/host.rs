...
use crate::resources::{RealBuffer, BufferType};
...
wasmtime::component::bindgen!({
    ...
    world: "plugin",
    with: {
        "vtx:api/stream-io/buffer": crate::resources::RealBuffer,
    }
});
...
impl vtx::api::stream_io::Host for StreamContext {
    fn open_file(&mut self, uuid: String) -> Result<Resource<RealBuffer>, String> {
        ...
        let rb = RealBuffer { inner: BufferType::File(file) };
        Ok(self.table.push(rb).map_err(|e| e.to_string())?)
    }

    fn create_memory_buffer(&mut self, data: Vec<u8>) -> Resource<RealBuffer> {
        let cursor = Cursor::new(data);
        let rb = RealBuffer { inner: BufferType::Memory(cursor) };
        self.table.push(rb).expect("Failed to allocate buffer resource")
    }
}
...
impl vtx::api::stream_io::HostBuffer for StreamContext {
    ...
    fn read(&mut self, resource: Resource<RealBuffer>, offset: u64, max_bytes: u64) -> Vec<u8> {
        let buffer = self.table.get_mut(&resource).expect("Invalid buffer handle");
...
impl vtx::api::sql::Host for StreamContext {
    fn execute(&mut self, statement: String, params: Vec<vtx::api::sql::DbValue>) -> Result<u64, String> {
        let conn = self.registry.get_conn().map_err(|e| e.to_string())?;

        let sql_params: Vec<Box<dyn ToSql>> = params.iter().map(|p| match p {
    ...
    fn query_json(&mut self, statement: String, params: Vec<vtx::api::sql::DbValue>) -> Result<String, String> {
        let conn = self.registry.get_conn().map_err(|e| e.to_string())?;
